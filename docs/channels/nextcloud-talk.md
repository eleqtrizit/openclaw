...
- The webhook listener admits at most 64 concurrent unauthenticated body reads; overflow requests receive `HTTP/1.1 429` with `Connection: close`. Requests on one keep-alive connection are answered in order, so a queued delivery's `200` acknowledgement always flushes before any overflow rejection closes the socket. Nextcloud Talk's retry behavior for refused deliveries is not verified: deployments that regularly exceed 64 concurrent reads should raise capacity (for example, connection limits on reverse proxies) or accept possible delivery loss during saturation.
